 ...
 * - PMserial
 * - SD_handler
 * - gsm::QuectelModem and gsm::GsmMqttClient
...
#include "utils/gsm/AtClient.h"
#include "utils/gsm/QuectelModem.h"
#include "utils/gsm/GsmMqttClient.h"
#include <TimeLib.h>
#include <ESP32Time.h>
...
void setup()
{
    ...
    if (DeviceConfigState.wifiConnected && DeviceConfigState.isHAConfigured && DeviceConfig.ha_enabled)
    {
        ...
        // Register the sensor entities this firmware exposes. The manager stays
        // generic: it only stores keys/names/units, not what they measure.
        haManager.addSensor("temperature", "Temperature", "temperature", "°C", "mdi:thermometer");
        haManager.addSensor("humidity", "Humidity", "humidity", "%", "mdi:water-percent");
        haManager.addSensor("pm1", "PM1", "pm1", "µg/m³", "mdi:blur");
        haManager.addSensor("pm25", "PM2.5", "pm25", "µg/m³", "mdi:blur");
        haManager.addSensor("pm10", "PM10", "pm10", "µg/m³", "mdi:blur");
...
bool sendDataViaGSM(const char *data, const int _pin, const char *url)
{
    if (!gsm_capable || !DeviceConfigState.gsmInternetAvailable || !modem.state().gprsConnected)
    {
    ...
    const int statuscode = modem.post(url, http_headers, 3, data, strlen(data));
...
void captureGSMInfo()
{
    ...
    gsm_info["Network Name"] = gsmRuntimeInfo.operatorName = modem.networkName();
    gsm_info["Signal Strength"] = gsmRuntimeInfo.signalStrength = modem.signalStrength();
    strcpy(gsmRuntimeInfo.simCcid, modem.state().simCcid);
    gsm_info["SIM ICCID"] = gsmRuntimeInfo.simCcid;
    gsm_info["Model ID"] = gsmRuntimeInfo.modelId = modem.modelId();
    gsm_info["Firmware Version"] = gsmRuntimeInfo.firmwareVersion = modem.firmwareVersion();
    gsm_info["IMEI"] = gsmRuntimeInfo.imei = modem.imei();
}
...
void initializeAndConfigGSM()
{
    ...
    DeviceConfigState.gsmInternetAvailable = false;
    CommsManagerState.gsmOnline = false;
    CommsManagerState.allCommsUnavailable = !CommsManagerState.wifiOnline;
    ...
    // Store the CA certificate in the modem's UFS.
    if (!modem.fileSystem().exists(MOZILLA_PEM_FILENAME))
    {
        Serial.print("Writing modem file: ");
        Serial.println(MOZILLA_PEM_FILENAME);

        if (!modem.fileSystem().writeText(MOZILLA_PEM_FILENAME, MOZILLA_CA_BUNDLE))
        {
            Serial.println("Failed to write to file");
        }
        else
        {
            if (!modem.setCaCertificate(MOZILLA_PEM_FILENAME))
            {
    ...
    {
        Serial.println("Modem file transfer lost synchronization; retry GSM initialization");
        DeviceConfigState.gsmConnected = false;
        return;
    }

    const gsm::NetworkMode modes[] = {gsm::NetworkMode::Auto, gsm::NetworkMode::G2, gsm::NetworkMode::G4};
    bool network_registered = false;

    for (auto mode : modes)
    {
        if (modem.setNetworkMode(mode) && modem.registerToNetwork())
        {
    ...
    // GPRS initialization
    DeviceConfigState.gsmInternetAvailable = modem.initializeGprs();
    ...
    if (!DeviceConfigState.timeSet)
    {
        if (modem.getNetworkTime(time_buff, sizeof(time_buff)))
        {
    ...
    if (DeviceConfigState.gsmInternetAvailable && DeviceConfigState.isMQTTConfigured)
    {
        // initialize() reset the modem, so any previous GSM MQTT session is gone.
        const bool mqttReady = gsmMqtt.configure(MQTT_CLIENT_ID, 1, 1) &&
                               gsmMqtt.open(MQTT_CLIENT_ID, MQTT_BROKER, MQTT_PORT) &&
                               gsmMqtt.connect(MQTT_CLIENT_ID, esp_chipid, MQTT_USERNAME, MQTT_PASSWORD) &&
                               gsmMqtt.subscribe(MQTT_CLIENT_ID, 1, MQTT_SUBSCRIBE_TOPIC, 0);
        gsmMqttSubscriptionReady = mqttReady;
        CommsManagerState.mqttConnectionInitialized = mqttClient.connected() || mqttReady;
    }
    captureGSMInfo();
    modem.sleep();
}
...
#endif
#if defined(GSM_PIN)
    strncpy(DeviceConfig.sim_pin, GSM_PIN, sizeof(DeviceConfig.sim_pin) - 1);
...
bool pingServer(const char *server, uint16_t port, uint16_t timeout_ms)
{
    ...
    if (DeviceConfig.useGSM && DeviceConfigState.gsmConnected && modem.state().gprsConnected)
    {
        bool ping_success = modem.pingIp(server);
...
bool buildMQTTTelemetryPayload(char *mqtt_payload, size_t payload_size)
{
    try
    {
        ...
        if (gsm_capable && DeviceConfigState.gsmConnected)
        {
            ...
            gsm["connected"] = modem.state().gprsConnected;
            gsm["network_name"] = gsmRuntimeInfo.operatorName;
            gsm["signal_strength"] = gsmRuntimeInfo.signalStrength;
            gsm["imei"] = gsmRuntimeInfo.imei;
            gsm["model"] = gsmRuntimeInfo.modelId;
            gsm["firmware"] = gsmRuntimeInfo.firmwareVersion;
            gsm["sim_ccid"] = gsmRuntimeInfo.simCcid;
            gsm["battery_status"] = modem.batteryStatus();
        }
                          ...
                          const char *username = nullptr, const char *password = nullptr)
{
    ...
    if (!CommsManagerState.gsmOnline || !modem.state().gprsConnected)
    {
        Serial.println("sendMQTTTelemetry: GPRS not connected - cannot send telemetry");
        return false;
    }
    const gsm::MqttConnectionStatus status = gsmMqtt.status(client_id);
    if (status == gsm::MqttConnectionStatus::Disconnected)
    {
        Serial.println("sendMQTTTelemetry: Opening MQTT broker connection...");
        if (!gsmMqtt.configure(client_id, 1, 1) || !gsmMqtt.open(client_id, broker, port))
        {
            Serial.print("sendMQTTTelemetry: Failed to open MQTT broker - Error: ");
            Serial.println(modem.failures().mqttInitError);
            return false;
        }
    }
    // BrokerOpen still requires MQTT authentication; do not reopen the transport.
    if (status != gsm::MqttConnectionStatus::Connected)
    {
        if (client_id == MQTT_CLIENT_ID)
            gsmMqttSubscriptionReady = false;
        Serial.println("sendMQTTTelemetry: Connecting MQTT client...");
        if (!gsmMqtt.connect(client_id, esp_chipid, username, password))
        {
    ...
    if (!gsmMqtt.publish(client_id, 1, topic, mqtt_payload, 1, 0))
    {
                              ...
                              bool disconnect_after = true)
{
    ...
    if (disconnect_after && gsmMqtt.isBrokerConnected(client_id))
    {
        delay(500);
        Serial.println("initAndSendMQTTTelemetry: Disconnecting MQTT...");
        gsmMqtt.disconnect(client_id);
        if (client_id == MQTT_CLIENT_ID)
            gsmMqttSubscriptionReady = false;
    }
...
void buildDeviceInfoJSON()
{
    try
    {
        ...
        if (gsm_capable && (gsmRuntimeInfo.operatorName.length() > 0 ||
                            gsmRuntimeInfo.imei.length() > 0 ||
                            gsmRuntimeInfo.simCcid[0] != '\0'))
        {
            JsonObject gsm = device_info["GSM"].to<JsonObject>();

            if (gsmRuntimeInfo.operatorName.length() > 0)
                gsm["Operator Name"] = gsmRuntimeInfo.operatorName;
            if (gsmRuntimeInfo.signalStrength > 0)
                gsm["Signal Strength"] = gsmRuntimeInfo.signalStrength;
            if (gsmRuntimeInfo.networkTechnology[0] != '\0')
                gsm["Network Technology"] = gsmRuntimeInfo.networkTechnology;
            if (gsmRuntimeInfo.imei.length() > 0)
                gsm["IMEI"] = gsmRuntimeInfo.imei;
            if (gsmRuntimeInfo.modelId.length() > 0)
                gsm["Model"] = gsmRuntimeInfo.modelId;
            if (gsmRuntimeInfo.firmwareVersion.length() > 0)
                gsm["Firmware"] = gsmRuntimeInfo.firmwareVersion;
            if (gsmRuntimeInfo.simCcid[0] != '\0')
                gsm["SIM CCID"] = gsmRuntimeInfo.simCcid;
        }
...
void checkIncomingMQTTMessages()
{
    ...
    {
        const gsm::MqttConnectionStatus status = gsmMqtt.status(MQTT_CLIENT_ID);
        if (status != gsm::MqttConnectionStatus::Connected)
            gsmMqttSubscriptionReady = false;
        if (status == gsm::MqttConnectionStatus::Disconnected)
        {
            if (!gsmMqtt.configure(MQTT_CLIENT_ID, 1, 1) ||
                !gsmMqtt.open(MQTT_CLIENT_ID, MQTT_BROKER, MQTT_PORT))
            {
        ...
        if (status != gsm::MqttConnectionStatus::Connected)
        {
            ...
            if (!gsmMqtt.connect(MQTT_CLIENT_ID, esp_chipid, MQTT_USERNAME, MQTT_PASSWORD))
            {
        ...
        if (!gsmMqttSubscriptionReady)
        {
            gsmMqttSubscriptionReady = gsmMqtt.subscribe(MQTT_CLIENT_ID, 1, MQTT_SUBSCRIBE_TOPIC, 0);
            if (!gsmMqttSubscriptionReady)
            {
                Serial.println("receiveMQTTTelemetry: Failed to subscribe to configuration updates");
                return;
            }
        }

        if (gsmMqtt.hasBufferedMessage(MQTT_CLIENT_ID))
        {
            CommsManagerState.message_received = gsmMqtt.readBufferedMessage(MQTT_CLIENT_ID, incoming_topic_store, sizeof(incoming_topic_store), incoming_message_store, sizeof(incoming_message_store));
        }
    }
}
