...
bool QuectelModem::beginSerial(HardwareSerial &serial)
{
    ...
    bool commsInitialized = false;
    while (millis() - start < 60000UL)
    {
        at_.flush();
        if (at_.sendCommand("AT", "OK"))
        {
            commsInitialized = true;
            logger_.println("GSM module found!");
            break;
        }
        delay(250);
    }
    ...
    if (serialConfig_.debugEnabled)
    {
        at_.sendCommand("ATE1", "OK");
        at_.sendCommand("AT+CMEE=2", "OK");
    }
    else
    {
        at_.sendCommand("ATE0", "OK");
        at_.sendCommand("AT+CMEE=1", "OK");
    }
...
bool QuectelModem::initialize()
{
    ...
    if (serialConfig_.resetPin >= 0)
        hardwareReset(ResetSequence::LowHighLow);
    else
        softReset();
...
void QuectelModem::softReset()
{
    deactivateGprs();

    if (!at_.sendCommand("AT+CFUN=1,1", "OK"))
    {
        logger_.println("Soft resetting GSM with full functionality failed!");
        return;
    }

    logger_.println("Soft resetting the GSM module...");
...
void QuectelModem::hardwareReset(ResetSequence sequence, uint8_t timingDelayMs)
{
    if (serialConfig_.resetPin < 0)
        return;
